...
4. 通风接触机会富集:30 天轨迹 MLD 接触/近接触/outcrop 相对两组 matched controls
   显著增强(p=0.001–0.005,56/56 事件);60/90 天受左截断限。
...
- G1 **1/30° 沿等密面对准**:GLORYS null 的 OFES-side 落点——1/30° 能否分辨
  流向与等密面坡向的对准?→ 本夜已启动 w_along 审计(56 事件 × peak±3 天)。
- G2 **年度背景统计**:事件富集必须锚定到全年背景(SCV occupancy null + 检测器
  普查)。→ 年度目录工程试跑中(63 tile-days)。
...
| B | 年度 SCV 目录(G2)| 1 天试跑中 | 待外推(试跑 63 tile-days ≈ 4 h) |
...
> 锋面/应变过程先**形成并向下输送**水团异常,随后反气旋旋转/SCV-like 结构
> 负责**组织、滞留和延长寿命**——把"亚中尺度过程 vs SCV"从二选一改成两阶段。
