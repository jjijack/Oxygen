...
- OFES 同构:事件核心 McCoy 型虚拟剖面通过率 vs 同日背景 controls =
  ...
  [0.076, 0.221]、预声明的单侧 paired Wilcoxon p = 9.84e-5
  (透明报告双侧 p = 1.97e-4)**;pooled OR 用于直观展示
...
S5 背景抽样在 73 个系统日期中检出 549 个 Tier-1 对象,但背景面积占据率
仅 0.227%;按事件月份、1° 纬带与目标 σ0 匹配后,56 个事件核心的 Tier-1
预期命中数只有 0.237,观察到 0 个的参考概率约 79%。因此成熟闭合透镜不是
事件核心的常见解析形态,但事件核心 0/56 对 SCV-like/亚格子载体的区分力很低。
...
- grid-SCV 背景线:S5 系统抽样敏感性已完成(73 天 × 63 tile,
  4599/4599 tile-day,0 错误);Tier-1/weak/strong/well-resolved 对象数为
  549/309/153/98,occupancy 为 0.227%/0.119%/0.086%/0.082%。它是
  每 5 日系统抽样敏感性,不冒充 365 日 annual null。
...
| 子集 | n | 反气旋(负 Ro) | rotation-dominant | 双条件 | 水团 DO 贡献中位(μmol kg⁻¹) |
|---|---|---|---|---|---|
| 56 严格 | 56 | 52/56 | 29/56 | 27/56 | 71.7 |
| 其余 105 | 105 | 63/105 | 39/105 | 29/105 | 71.5 |
| 合计 | 161 | 115/161 | 68/161 | 56/161 | 71.6 |

- **水团异常分量在 161 普查上依旧主导**:水团 DO 贡献中位 71.6 μmol kg⁻¹
  (p25/p75 60.1/89.2),heave 贡献中位 11.1 μmol kg⁻¹;按
  `|water-mass|/(|water-mass|+|heave|)` 计算的绝对贡献分数中位为 78.2%,
  138/161 为 water-mass dominated。56 个严格事件则为 54/56 water-mass
  dominated、绝对贡献分数中位 86.2%。这说明结论在全部 quality 事件上
  复现,不是 deep 子集的偶然。
