...
def _configure_writable_runtime_caches() -> None:
    """Keep PET/Numba/Matplotlib caches out of the read-only home tree."""
...
def prepare_pet_grid(snapshot: EtaSnapshot, config: Mapping[str, Any]) -> PreparedPetGrid:
    """Build a PET grid, apply the locked filter, and add PET geostrophic speed."""
...
def hash_file_inventory(paths: Iterable[str | Path]) -> tuple[str, list[dict[str, Any]]]:
    """Hash an explicit ordered file inventory and return its audit rows."""
    ...
    for path in paths:
        ...
        rows.append({"path": str(source), "sha256": sha256_file(source), "size": source.stat().st_size})
    payload = json.dumps(rows, ensure_ascii=False, sort_keys=True).encode()
    return sha256_bytes(payload), rows


def package_versions() -> dict[str, str]:
    """Return versions used by the standalone runner."""
...
def runtime_environment_manifest() -> dict[str, Any]:
    """Capture interpreter, package, and PET source identity for a run."""

    import py_eddy_tracker

    module_path = Path(py_eddy_tracker.__file__).resolve()
    return {
        "python": sys.version,
        "python_executable": sys.executable,
        "platform": platform.platform(),
        "packages": package_versions(),
        "py_eddy_tracker_module": str(module_path),
        "py_eddy_tracker_module_sha256": sha256_file(module_path),
        "py_eddy_tracker_version": py_eddy_tracker.__version__,
    }
