...
- 日期:2026-08-24（科学收口更新）
- 状态:论文写作基线；后续代码整理不改变本文数字，新增分析须另行标记
...
> 可以重叠发生,共同构成从通风水团到闭合 SSH 目录难以捕捉的次表层载体的连续过程。

一句话:**"全球识别 SCV 为最强的组织载体,OFES 揭示富氧水团如何从通风层
进入深层并被闭合 SSH 目录难以捕捉的次表层结构组织和保持。"**
...
- OFES 对偶富集 Δ=0.144 [0.076,0.221]；预设方向的单侧 p=9.8×10⁻⁵，
  透明报告双侧 p=1.97×10⁻⁴；背景 20/4480
- OFES 表层旋转印记与闭合 SSH 目录系统脱钩(26/29 表层—深层 Ro 同号;
  PET strict 0/31、rotation 0/16、quality 0/58 analysis-eligible;双侧负 null
  两口径 paired n=31 且 CI 全在 0 左)
...
- Zhu 与 grid-SCV 不覆盖事件核心；S5 在背景中检出 549 个 Tier-1 对象，
  但其面积占据率仅 0.227%，按事件月份×纬带×σ0 匹配后 56 个核心的预期
  命中数仅 0.237，观察到 0 个的参考概率约 79%。因此成熟闭合透镜不是事件
  核心的常见解析形态，而 0/56 本身不足以检验 SCV-like/亚格子载体机制
...
4. **OFES 事件群体** — 对偶富集(P(SCV|DO) 呼应全球 P(DO|SCV));表层有旋转
   印记却与闭合 SSH 目录脱钩(26/29 Ro 同号;PET strict 0/31、quality 0/58
   eligible+双侧负 null);剖面签名与成熟闭合透镜的层级差异;水团分解
...
7. **尺度连接与过程分辨率** — w_along 的群体背景与 resolved 子集、
   GLORYS/OFES 的 SCV 重现能力、grid-SCV 与 S5 稀疏背景占据率；解释为什么
   成熟闭合透镜的零包含功效很低，而剖面富集、原生速度支持和轨迹过程仍有信息
...
| OFES 事件核心富集 McCoy-compatible 剖面 | 事件等权 core−background | +0.144 [0.076,0.221]；单侧 p=9.8×10⁻⁵，双侧 p=1.97×10⁻⁴ | 方向假设与透明双侧值同时报告 | S |
| OFES 表层旋转印记与闭合 SSH 涡目录脱钩 | 26/29 Ro 同号 + PET local/annual null | PET 0/31；双侧 p=8.9×10⁻⁵/1.7×10⁻⁶ | 表层有动力响应但不形成目录闭合涡;域外事件单列不可判定 | S(观测事实) |
...
| 成熟闭合网格 SCV 不常覆盖事件核心 | grid-SCV containment + S5 | 0/56；S5-matched 期望 0.237，P(0)≈0.79 | 低占据率、低功效；不否定 SCV-like 载体 | B |

## 九、当前科学收口

截至 2026-08-24，没有仍在运行且会改变正文主线的正式科学分析。S5 已完成，
PET、grid-SCV、通风史、三维轨迹、w_along、阶段桥接与 161-event 普查均已有
正式报告。现在可以开始写作；后续工作主要是代码合并、图面取舍、局部敏感性或
审稿人驱动的补充，不应把历史 lock 中的预期当成最终结果。
